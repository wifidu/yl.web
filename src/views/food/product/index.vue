  ...
  <div class="app-container" >
      ...
      <el-card style="margin-top: 15px" shadow="never">
        ...
        <el-form :inline="true" :model="listQuery" size="small" label-width="180px" style="margin-top: 30px">
          <el-form-item label="单品名称：">
            <el-input v-model="listQuery.food_name" class="input-width" placeholder="必填" style="width:300px"></el-input>
          </el-form-item>
          <el-form-item label="单品价格：">
            <el-input v-model="listQuery.food_price" class="input-width" placeholder="必填" style="width:300px"></el-input>
          </el-form-item>
          <el-form-item label="单品状态：">
            <el-input v-model="listQuery.food_type" class="input-width" placeholder="必填" style="width:300px"></el-input>
          </el-form-item>
          <el-form-item label="所属类别：">
            <el-input v-model="listQuery.subordinate_species" class="input-width" placeholder="必填" style="width:300px"></el-input>
          </el-form-item>
    ...
    <div class="table-container" style="width: 100%">
      <el-table ref="orderTable"
        ...
        <el-table-column label="单品名称" width="180" align="center">
          <template slot-scope="scope">{{scope.row.food_name}}</template>
        </el-table-column>

        <el-table-column label="单品价格" width="180" align="center">
          <template slot-scope="scope">{{scope.row.food_price}}</template>
        </el-table-column>
        <el-table-column label="单品状态" width="" align="center">
          <template slot-scope="scope" v-if="change==false">{{scope.row.food_type==0?'无':'有'}}</template>
    ...
    methods:{
      ...
      handleSearchList() {
        ...
        if(this.searchList.member_name == ''){
          ...
          getList(this.searchList,"/diet-manage/food-manage/search").then(response => {
      ...
      handleSaveList() {
        ...
        appointList(this.listQuery,'/diet-manage/food-manage/').then(response=>{
          this.$message({
            type: 'success',
            message: '退住成功!',
            duration:1000
      ...
      handleDeleteDetail(index,row){
        this.$confirm('是否要进行删除操作?', '提示', {
          ...
          deleteOrder('/diet-manage/food-manage/',row.id).then(response=>{
            this.getList();
            this.$message({
              type: 'success',
              message: '删除成功!'
      ...
      handleBatchOperate(){
        ...
        this.$confirm('是否要进行该删除操作?', '提示', {
          ...
          for(let i=0;i<ids.length;i++){
            ...
            deleteOrder('/diet-manage/food-manage/',ids[i]).then(response=>{
              this.$message({
                message: '删除成功！',
                type: 'success',
                duration: 1000
      ...
      getList(){
        this.listLoading=true;
        getList(this.query_string,'/diet-manage/food-manage/list').then(response => {
          this.listLoading = false;
