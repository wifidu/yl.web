    ...
    methods: {
      ...
      handleSearchList() {
        ...
        if(this.searchList.member_name === '' && this.searchList.phone_number === ''){
          ...
          getList(this.search_name,"/diet-manage/package-manage/search/").then(response => {
          ...
          getList(this.search_phone,"/diet-manage/package-manage/search").then(response => {
            this.listLoading = false;
            this.list = response.data;
            console.log(response)
          });
      ...
      handleAdd(){
        this.$router.push({path:'/food/package_new',query:{id:0,edit:false}})
      },
      handleChangeOrder(index, row){
        this.$router.push({path:'/food/package_edit',query:{id:row.id,edit:false}})
      },
      handleViewOrder(index, row){
        this.$router.push({path:'/food/package_view',query:{id:row.id,edit:true}})
      },
      ...
      getList() {
        this.listLoading = true;
        getList(this.dataList,"/diet-manage/package-manage/list").then(response => {
          this.listLoading = false;
      ...
      deleteOrder(ids){
        this.$confirm('是否要进行该删除操作?', '提示', {
          ...
          deleteOrder('/diet-manage/package-manage/',ids).then(response=>{
            this.$message({
              message: '删除成功！',
              type: 'success',
              duration: 1000
