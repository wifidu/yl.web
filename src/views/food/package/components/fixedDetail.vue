      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="add_list" :rules="rules" size="small" label-width="180px">
          ...
          <el-form-item label="套餐价格：" prop="package_price">
            <el-input v-model="add_list.package_price" :disabled="edit" class="input-width" style="width:320px"></el-input>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created(){
      ...
      if(this.id){
        getList('/diet-manage/package-manage/',this.id).then(response => {
    ...
    methods:{
      handleBack(){
        this.$router.push({path:'/food/package'})
      },
      ...
      addList(){
        ...
        if (this.add_list.food_name !== "" && this.add_list.food_price !== "" && this.add_list.food_type !== null && this.add_list.subordinate_species !== ""){
          update('/diet-manage/package-manage/',this.add_list).then(response => {
